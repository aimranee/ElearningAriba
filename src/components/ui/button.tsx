...
const buttonVariants = cva(
  "group/button inline-flex shrink-0 items-center justify-center rounded-lg border border-transparent bg-clip-padding text-sm font-medium whitespace-nowrap transition-[transform,box-shadow,border-color,background-color] duration-[var(--duration-base)] ease-[var(--ease-brand)] outline-none select-none in-data-[density=compact]:h-7 in-data-[density=compact]:text-[0.8rem] focus-visible:border-ring focus-visible:ring-3 focus-visible:ring-ring/50 active:not-aria-[haspopup]:translate-y-px disabled:pointer-events-none disabled:opacity-50 aria-invalid:border-destructive aria-invalid:ring-3 aria-invalid:ring-destructive/20 dark:aria-invalid:border-destructive/50 dark:aria-invalid:ring-destructive/40" +
    ...
    " data-[loading=true]:pointer-events-none data-[loading=true]:opacity-70 [&_svg]:pointer-events-none [&_svg]:shrink-0 [&_svg:not([class*='size-'])]:size-4 [&_svg[data-slot=button-spinner]]:hidden data-[loading=true]:[&_svg[data-slot=button-spinner]]:block",
  {
    variants: {
      variant: {
        /* why (D-06/D-12): the default variant is the maquette's pill CTA —
           rounded-full, wearing --shadow-brand at rest and escalating to
           --shadow-4 on hover. data-magnetic is a pass-through: no new prop
           is declared, {...props} already spreads onto ButtonPrimitive, so a
           call site writes data-magnetic="true" and the D-12 island selects
           [data-slot="button"][data-magnetic="true"]. */
        default:
          "rounded-full bg-primary text-primary-foreground shadow-[var(--shadow-brand)] hover:bg-primary/80 hover:shadow-[var(--shadow-4)] active:shadow-[var(--shadow-1)]",
        outline:
