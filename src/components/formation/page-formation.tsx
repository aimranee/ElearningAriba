...
/*
 * why (#21): the Formation page body, rendered by both route files —
 * (public)/formation in French and (public-en)/en/training in English — so
 * the two copies cannot drift. Each route file keeps its own `revalidate`
 * (segment config must be declared in the route file itself). Links into
 * the booking tunnel keep their French URLs: the tunnel is French until
 * Phase B.
 */
...
export async function PageFormation({ locale }: { locale: Locale }) {
  ...
  const [sectionResult, itemsResult, programmeSectionResult, programmeItemsResult] =
    await Promise.all([
      getSection("page-formation", locale),
      getSectionItems("page-formation", locale),
  ...
  const faits: Fait[] = [
    {
      ...
      valeur: formation.faits.parcoursValeur
        .replace("{modules}", formatNumber(modules.length, locale))
        .replace("{heures}", formatHours(totalHeures, locale)),
    },
  ];
    ...
    <>
      ...
      <Section id="programme" tone="wash" data-section="programme">
        ...
        <div className="mt-10 lg:grid lg:grid-cols-[minmax(0,1fr)_17rem] lg:items-start lg:gap-8">
            ...
            <div className="grid gap-5 md:grid-cols-2">
              {modules.map((module, index) => (
                <Reveal key={module.id} dataD={((index % 5) + 1) as 1 | 2 | 3 | 4 | 5}>
                  <CarteModule module={module} index={index} locale={locale} />
                </Reveal>
              ))}
            </div>
          ...
          <AideRdv locale={locale} className="hidden lg:sticky lg:top-[104px] lg:flex" />
        </div>
      </Section>
      ...
      <Temoignages locale={locale} />

      {/* 7. RDV band above the footer. */}
      <BandeauRdv locale={locale} />
      ...
      <BarreRdv
        href={RDV_DECOUVERTE_HREF}
        titre={formation.aide.titre}
        texte={texteAideRdv(locale)}
        action={common.actions.prendreRdv}
      />
    </>
  );
}
