...
/*
 * why (#19): the About page body, rendered by both route files —
 * (public)/a-propos in French and (public-en)/en/about in English — so the
 * two copies cannot drift. Each route file keeps its own `revalidate`
 * (segment config must be declared in the route file itself).
 */
...
export async function PageAPropos({ locale }: { locale: Locale }) {
  const common = getMessages(locale, "common");
  const [sectionResult, itemsResult] = await Promise.all([
    getSection("page-a-propos", locale),
    getSectionItems("page-a-propos", locale),
  ]);
